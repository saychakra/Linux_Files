...
def generate(frames, text_lines):
	...
	# Generate a filename based on the current time
        ...
        ##################################################################################################################### 
        ####personal modifications to remove the existing data in the snapshots folder and storing only the latest scan######
        ##################################################################################################################### 

        # deleing the existing snapshots and storing only the current snapshot
	mydir = "/lib/security/howdy/snapshots"
	removal_list = [ f for f in os.listdir(mydir) if f.endswith(".jpg") ]
